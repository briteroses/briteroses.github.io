...
Looking at our math, we do need to be careful about our construction of aligned and unaligned distributions. Less formally, the reward correlation conditions in Theorem 2 state that, the higher likelihood a response is in the aligned distribution, the higher the reward should be on average (and the higher likelihood a response is in the *unaligned* distribution, the *lower* the reward should be on average). This sounds fairly reasonable, but these conditions might not always be true in practice. Fortunately, the reward finiteness condition should always hold; in DPO, finiteness is clear from the form of the implicit reward used.

There's a link here between DPO-based safety finetunes and adversarial attacks (jailbreaks) which may be fairly obvious, but that I still want to highlight. Because we can never expunge harmful generations, we can re-amplify the probability of obtaining harmful trajectories from our post-DPO model with various attack methods, from [high-temperature decoding](https://arxiv.org/abs/2310.06987) to [discrete-optimized suffixes](https://arxiv.org/pdf/2307.15043) to [randomized prompt fuzzing with a high attack budget](https://arxiv.org/abs/2412.03556). Additionally, our analysis only extends to prompt-conditioned output distributions, and we can't say anything definitive about model behavior under prompts out-of-distribution of those in our DPO data. Under the [mismatched generalization](https://arxiv.org/abs/2307.02483) hypothesis for jailbreaks, safety finetunes lose effectiveness on out-of-distribution prompts; this proposition illuminates what may be happening in the blind spots of our analysis.